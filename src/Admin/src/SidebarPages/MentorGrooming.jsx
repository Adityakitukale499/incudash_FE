...
const MentorGrooming = () => {
    ...
    const toggleIsDeactivated = (id) => {
        const updatedMentors = mentors.map((mentor) => {
            if (mentor.id === id) {
                const updatedMentor = { ...mentor, isDeactivated: !mentor.isDeactivated };
                putData(`mentor-groomings/${id}`, { isDeactivated: updatedMentor.isDeactivated })
                    .then((response) => {
                        console.log(response.data);
                    })
                    .catch((error) => {
                                    ...
                                    <CardActionArea sx={{ display: "flex", justifyContent: "left" }}>
                                        ...
                                        <CardContent>
                                            ...
                                            <Typography variant="body1" color="primary" sx={{ fontWeight: 600 }}>
                                                {e?.mentorInfo?.designation} At {e?.mentorInfo?.companyName}
                                            </Typography>
